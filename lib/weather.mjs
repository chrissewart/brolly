...
// Intensity colour ramp (light blue → blue → teal → yellow → orange → red),
// shared with the radar overlay (index.html's precipStyle()) so "darker/
// hotter = heavier rain" means the same thing everywhere in the app rather
// than each view inventing its own colour language.
export const PRECIP_SCALE = [
  [0.5,      '#a9d9f2'],  // drizzle
  [1.5,      '#4f9fd8'],  // light
  [3,        '#2fae8b'],  // moderate
  [6,        '#e0c22f'],  // moderate–heavy
  [12,       '#e0812f'],  // heavy
  [Infinity, '#d1372f'],  // very heavy / extreme
];

export function rainColor(mm) {
  const v = mm == null || mm < 0 ? 0 : mm;
  return PRECIP_SCALE.find(([th]) => v < th)[1];
}

// Tuned separately from the radar overlay's own opacity curve — this is a
// ~30px-tall glyph, not a full map layer, so it wants a higher opacity floor
// to actually be visible at that size rather than matching the map's subtler
// ramp exactly.
export function rainOpacity(mm) {
  const v = mm == null || mm < 0 ? 0 : mm;
  return Math.min(0.85, 0.35 + Math.sqrt(v / 8) * 0.5);
}
...
export function sparkline(h, dateStr, tlo, thi) {
  ...
  // One hourly rect per known-probability hour, rather than one continuous
  // area fill: height still encodes probability (matches the outline path
  // above it exactly), but each bar's own colour/opacity now encodes
  // predicted mm — an SVG <path> can't vary colour along its length, so
  // amount needs its own per-hour segments to be visually distinct from
  // probability (see roadmap item 6).
  const rainBars = rp.map(([x,y,mm]) =>
    `<rect x="${x}" y="${100-y}" width="0.92" height="${y}" fill="${rainColor(mm)}" fill-opacity="${rainOpacity(mm).toFixed(2)}"/>`
  ).join("");
