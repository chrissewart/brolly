...
// Bucketed against the same UK Met Office hourly rain-rate categories used
// to calibrate rainColor() — light <2.5mm/h → 1 drop, moderate 2.5–10mm/h
// → 2, heavy/violent ≥10mm/h → 3 — so the droplet count means the same
// "how heavy" as the sparkline/radar colour and the WMO code label instead
// of a fourth, independently-invented scale. The old thresholds (<0.5/<2)
// were exactly that: 2.4mm hit 3 drops there, "moderate" teal under the
// old colour bands, and "Light rain" from the WMO label, three answers to
// the same question at once — this is what that mismatch was.
export function rainBars(mm, code, prob) {
  let n = 0;
  if (mm != null && mm > 0)
    n = mm < 2.5 ? 1 : mm < 10 ? 2 : 3;
...
export const NEARBY_MILES = 3;

// Haversine, in miles. Pure so the "don't add a near-duplicate GPS
// location" rule (index.html) can be unit tested without a real GPS fix.
...
// The closest already-saved location to `pos`, or null if there are none
// yet (first-ever location has nothing to be "near"). Used both to block a
// near-duplicate "Add current location" and to name which existing
// location it was too close to in the message shown for that case.
export function nearestLocation(pos, locations) {
  if (!locations.length) return null;
  let best = null;
  for (const loc of locations) {
    const miles = milesBetween(pos, loc);
    if (!best || miles < best.miles) best = { loc, miles };
  }
  return best;
}

// Open-Meteo's geocoding search has no explicit relevance/rank param beyond
// its own internal matching, so "Newcastle" (searched in the UK, e.g.) can
// come back with obscure hamlets ranked ahead of the actual major city the
// user almost certainly meant. Sorting by population descending (missing
// population - it's absent for plenty of small places - sorts last, not
// first, and ties/all-missing keep the API's own relative order since
// Array.prototype.sort is stable) surfaces the place someone's actually
// looking for without needing a real relevance signal from the API itself.
export function rankGeocodeResults(results) {
  return [...results].sort((a, b) => (b.population || 0) - (a.population || 0));
}

// Open-Meteo geocoding API result -> {lat, lon, name}. Pure formatting
// (which admin/country fields to join and how) pulled out so it's testable
// without a network call — the API returns fields inconsistently (small
// countries/city-states may have no admin1, etc).
export function formatGeocodeResult(r) {
  return {
    lat: r.latitude,
    lon: r.longitude,
