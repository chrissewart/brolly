...
// Returns "(NN°)" when apparent_temperature rounds to something different
// from the actual temperature, "" when they round the same (most of the
// time — no point cluttering a row/stat with a duplicate number) or when
// `feels` is unavailable (demo fixtures don't carry apparent_temperature).
export function feelsLikeSuffix(actual, feels) {
  if (feels == null || actual == null) return "";
  const a = Math.round(actual), f = Math.round(feels);
  return a === f ? "" : `(${f}°)`;
}
